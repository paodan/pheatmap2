...
draw_annotations = function(converted_annotations, border_color, gaps, fontsize, horizontal){
    ...
    if(horizontal){
        ...
        res = rectGrob(x = coord$x, y = coord$y, width = coord_x$size, height = unit(fontsize, "bigpts"),
        ...
        coord = expand.grid(y = y, x = x)
        res = rectGrob(x = coord$x, y = coord$y, width = unit(fontsize, "bigpts"), height = coord_x$size,
                       gp = gpar(fill = converted_annotations, col = border_color))
    }
...
draw_annotation_legend = function(annotation, annotation_colors, border_color, ...){
    ...
    for(i in names(annotation)){
        ...
        if(is.character(annotation[[i]]) | is.factor(annotation[[i]])){
            ...
            res[[paste(i, "r")]] = rectGrob(x = unit(0, "npc"), y = yy, hjust = 0, vjust = 1,
                                            height = 2 * text_height, width = 2 * text_height,
                                            gp = gpar(col = border_color, fill = annotation_colors[[i]]))
            res[[paste(i, "t")]] = textGrob(names(annotation_colors[[i]]), x = text_height * 2.4,
                                            y = yy - text_height, hjust = 0, vjust = 0.5, gp = gpar(...))
            ...
            res[[paste(i, "r")]] = rectGrob(x = unit(0, "npc"), y = yy, hjust = 0, vjust = 1,
                                            height = h, width = 2 * text_height,
                                            gp = gpar(col = NA, fill = colorRampPalette(annotation_colors[[i]])(4)))

            res[[paste(i, "r2")]] = rectGrob(x = unit(0, "npc"), y = y, hjust = 0, vjust = 1,
                                             height = 8 * text_height, width = 2 * text_height,
...
dist_mat = function (mat, distance, method){
  if (!(method %in% c("ward.D", "ward.D2", "ward", "single",
                      "complete", "average", "mcquitty", "median", "centroid"))) {
    stop("clustering method has to one form the list: 'ward', 'ward.D', 'ward.D2', 'single', 'complete', 'average', 'mcquitty', 'median' or 'centroid'.")
  }
  if (!(distance[1] %in% c("correlation", "euclidean", "maximum",
...
optimizeLeavesOrder = function(distance, hcluster){
  co <- cba::order.optimal(distance, hcluster$merge)
  ho <- hcluster
  ho$merge <- co$merge
  ho$order <- co$order
  return(ho)
...
#' @param annotation_colors_NA specify the color of the NA in the annotation_col and
#' annotation_row. The default is grey (#808080).
...
pheatmap2 = function(mat,
                    ...
                    silent = FALSE, na_col = "#DDDDDD", annotation_colors_NA = "#808080", ...){
    ...
    if((class(cluster_rows) == "hclust") || cluster_rows){
        if(class(cluster_rows) == "hclust"){
            ...
            # optimize the order of leaves
            if (clustering_rows_optimalOrder){
              tree_row = optimizeLeavesOrder(distance = dist_mat(mat, distance = clustering_distance_rows, method = clustering_method),
                                             hcluster = tree_row)
            }
        }
    ...
    if((class(cluster_cols) == "hclust") || cluster_cols){
        if(class(cluster_cols) == "hclust"){
            ...
            # optimize the order of leaves
            if (clustering_cols_optimalOrder){
              tree_col = optimizeLeavesOrder(distance = dist_mat(t(mat), distance = clustering_distance_cols, method = clustering_method),
                                             hcluster = tree_col)
            }
        }
